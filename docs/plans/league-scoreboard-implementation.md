...
### Goal
Build the "Self-Healing" capability *before* we build the Read API. We must assume Redis is empty on every deploy.
...
- [ ] Implement `refreshLeagueRanking(leagueId, tournamentId)`:
    ...
    -   *Note:* Handle the case where the key doesn't exist (first run).
