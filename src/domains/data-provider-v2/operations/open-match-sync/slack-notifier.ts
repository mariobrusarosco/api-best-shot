...
export const notifyOpenMatchSyncExecution = async (input: {
  ...
  try {
    ...
    Logger.error(error as Error, {
      ...
      operation: 'notifyOpenMatchSyncExecution',
...
export const buildOpenMatchSyncSlackPayload = (input: {
  requestId: string;
  tournamentId: string;
  tournamentLabel: string;
  status: OpenMatchSyncWorkflowStatus;
  ...
  const blocks: SlackBlock[] = [
    {
      ...
      text: {
        type: 'plain_text',
        text: `${statusMeta.emoji} Open Match Sync ${statusMeta.label}`,
        emoji: true,
      },
    },
    {
  ...
  return {
    text: `${statusMeta.emoji} Open Match Sync ${statusMeta.label} - ${input.tournamentLabel}`,
...
const buildReportText = (reportUpload?: OpenMatchSyncReportUploadResult): string => {
  if (!reportUpload) {
...
const formatSummary = (summary: TournamentOpenMatchSyncSummary): string => {
  ...
  if (summary.providerNotFoundMatches > 0) {
    parts.push(`${summary.providerNotFoundMatches} provider not found`);
  }

  if (summary.providerMissingEventMatches > 0) {
    parts.push(`${summary.providerMissingEventMatches} missing event`);
  }

  if (summary.openMatches > 0) {
    parts.push(`${summary.openMatches} still open`);
  }
