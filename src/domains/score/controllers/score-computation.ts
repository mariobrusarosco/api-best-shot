import { DB_SelectGuess } from '../../../services/database/schema';
...
const hasGuessedExactScore = (guess: DB_SelectGuess, match: any) =>
  guess.homeScore === match.homeScore && guess.awayScore === match.awayScore;

const hasGuessedMatchResult = (guess: DB_SelectGuess, match: any) => {
...
export const analyzeScore = (guess: any, match: any): IScore => {
  ...
  const MATCH_RESULT = hasGuessedMatchResult(guess, match)
    ? SCORE_MAPPER.MATCH_RESULT
    : 0;
