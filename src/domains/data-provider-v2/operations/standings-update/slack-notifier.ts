...
export const notifyStandingsUpdateExecution = async (input: {
  ...
  try {
    ...
    Logger.error(error as Error, {
      ...
      operation: 'notifyStandingsUpdateExecution',
...
export const buildStandingsUpdateSlackPayload = (input: {
  ...
  const blocks: SlackBlock[] = [
    {
      ...
      text: {
        type: 'plain_text',
        text: `${statusMeta.emoji} Standings Update ${statusMeta.label}`,
        emoji: true,
      },
    },
    {
  ...
  return {
    text: `${statusMeta.emoji} Standings Update ${statusMeta.label} - ${input.tournamentLabel}`,
...
const buildReportText = (reportUpload?: StandingsUpdateReportUploadResult): string => {
  if (!reportUpload) {
